...
if ($_POST) {
    ...
    $paginacion  = $measurements->getPagination();
    ...
    if ($termino != '') {
        $data = $measurements->getSearch($termino);
    } else {
        $data = $measurements->getNombre('almacenes');
        //$data = $measurements->getAll($empezarDesde, $filasPagina);
    }
    $result = $measurements->showSelect($data);
    //echo($result);
